...
| CoAP commands **from the LAN to a node** (`ac on`, etc.) send fine and the command executes (LED/AC state actually changes), but **no ACK/response is ever received**, every attempt retries 4x over ~70-85s then times out | **Resolved 2026-08-16.** Side effect of the border router's own `net.ipv6.conf.all.forwarding=1` fix (see the forwarding row below): enabling forwarding makes the kernel stop accepting Router Advertisements on that interface by default, so the border router's own LAN interface (`wlan0`) eventually lost its global address/route once its previously-learned one expired — it had no way to route the node's response back out to the LAN. Confirmed via a `tcpdump -i any -n port 5683` capture on the border router spanning both `wpan0` and `wlan0`: the node's ACK arrived on `wpan0` every time, never left via `wlan0`. Fix: `sudo sysctl -w net.ipv6.conf.wlan0.accept_ra=2` on the border router (means "accept RAs even though forwarding is on"), then force a fresh RA (`sudo rdisc6 wlan0`) | [border-router-native.md](./border-router-native.md) |
| CoAP commands **from the LAN to a node** fail completely — send, retry 4x over ~70-85s, give up, **zero response, and the command itself never executes either** — while the **reverse** direction (node's own UDP announce, SRP registration) works fine | **Resolved 2026-08-16.** Root cause was on the LAN client host (e.g. hermes), not the border router: `net.ipv6.conf.<iface>.accept_ra_rt_info_max_plen` (the per-interface value, not `all`) was `0`, so it never learned a route to the mesh's OMR prefix from the border router's Router Advertisement, and its CoAP packets went out its default internet gateway instead | [border-router-native.md](./border-router-native.md) |
| A LAN host has no route to a Thread mesh's OMR prefix (`ip -6 route show` missing an `fd..::/64 ... proto ra` entry) even though the border router is advertising it | `net.ipv6.conf.<iface>.accept_ra_rt_info_max_plen` — the **per-interface** value — is `0`. Setting `net.ipv6.conf.all.accept_ra_rt_info_max_plen` alone does **not** propagate to an already-existing interface's own value on this kernel; set the per-interface one explicitly, then force a fresh RA (`sudo rdisc6 <iface>`, needs `ndisc6`) rather than waiting for the next periodic one | [border-router-native.md](./border-router-native.md) |
| A Linux box acting as an IPv6 router (`forwarding=1` on some interface) stops picking up/renewing its own global address or default route on its LAN-facing interface — `ip -6 addr show` shows only a link-local address, `ip -6 route get <off-link-addr>` says `Network is unreachable` | Enabling `forwarding=1` makes the kernel treat that interface as a router, and by default (`accept_ra=1`) a router stops accepting Router Advertisements on it — set `net.ipv6.conf.<iface>.accept_ra=2` to keep accepting RAs regardless of forwarding state. Often doesn't fail immediately (an already-learned address/route survives until its lease expires), which delays the symptom well past when forwarding was actually turned on | [border-router-native.md](./border-router-native.md) |
